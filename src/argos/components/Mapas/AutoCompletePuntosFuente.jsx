...
export const AutoCompletePuntosFuente = ({data,handleFuentePuntosIdentificados}) => {

  /* 
  Como el nombre indica el componente es un input que recibe de datos de entrada 
  un catalogo, en concreo de la fuente y delitos y el handler para almacenar la respuesta seleccionada
*/
   const [items, setItems] = useState([])
  let itemsFD = []
  data.forEach(dato => {
    itemsFD.push({id: dato.indice, name: dato.fuente})
} )
  ...
  const handleOnSelect = (item) => {
    // the item selected
    console.log(item)
    handleFuentePuntosIdentificados(item)
  }
  ...
  const handleOnClear = () => {
    handleFuentePuntosIdentificados({id:9999,name: ''})
  }
