...
const { VITE_BACKEND_SERVER_CATALOGOS, VITE_BACKEND_SERVER_CATALOGOS_LOCAL } = getEnvVariables()
//Se crea y exporta la funcion que genera nuestro intermediario entre el frontend
//y el backend
export const catalogosApi = axios.create({
    baseURL: VITE_BACKEND_SERVER_CATALOGOS
});
...
catalogosApi.interceptors.request.use( config => {
    ...
    if (url.includes('187.216.250.252')) {
        config.baseURL = VITE_BACKEND_SERVER_CATALOGOS;
    } else if (url.includes('172.18.110.90')) {
        config.baseURL = VITE_BACKEND_SERVER_CATALOGOS_LOCAL;
