...
const { VITE_BACKEND_SERVER_GRAFICAS, VITE_BACKEND_SERVER_GRAFICAS_LOCAL } = getEnvVariables()
//Se crea y exporta la funcion que genera nuestro intermediario entre el frontend
//y el backend
export const graficasApi = axios.create({
    baseURL: VITE_BACKEND_SERVER_GRAFICAS
});
...
graficasApi.interceptors.request.use( config => {
    const url = window.location.href;

      if (url.includes('187.216.250.252')) {
        config.baseURL = VITE_BACKEND_SERVER_GRAFICAS;
    } else if (url.includes('172.18.110.90')) {
        config.baseURL = VITE_BACKEND_SERVER_GRAFICAS_LOCAL;
    }
...
// con status 400, manejadas por el servidor sacar al usuario no autenticado de la aplicación
 graficasApi.interceptors.response.use(
     response => response,
     error => {
